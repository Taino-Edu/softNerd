...
import {
  Bot, CheckCheck, CircleUserRound, Loader2, MessageCircle, PauseCircle,
  PlayCircle, Search, Send, Sparkles, UserRound, WifiOff,
} from 'lucide-react'
...
export default function WhatsAppInbox({ compact = false, initialPhone, onUnreadChange }: Props) {
  ...
  return (
    <div className={clsx(
      'grid min-h-0 overflow-hidden border border-surface-500 bg-surface-800',
      compact ? 'h-full rounded-xl grid-cols-1' : 'h-[calc(100vh-142px)] min-h-[560px] rounded-2xl lg:grid-cols-[320px_minmax(380px,1fr)_270px]',
    )}>
      {showList && (
              ...
              <button key={conversation.phone} onClick={() => setSelectedPhone(conversation.phone)}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-1 mt-0.5">
                    {conversation.humanMode ? <UserRound size={11} className="text-amber-400" /> : <Bot size={11} className="text-brand-400" />}
      ...
      {showChat && (
        <section className={clsx('min-h-0 flex flex-col bg-[#0f1115]', !compact && 'border-r border-surface-500')}>
          {selected ? (
            <>
              <header className="h-16 px-4 flex items-center gap-3 border-b border-surface-500 bg-surface-800/90 shrink-0">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-gray-500">+55 {selected.phone} · {selected.humanMode ? 'Maikon atendendo' : 'Bot atendendo'}</p>
                </div>
                <button onClick={toggleMode} title={selected.humanMode ? 'Devolver ao bot' : 'Assumir conversa'}
      ...
      {!compact && (
        <aside className="hidden lg:flex min-h-0 flex-col p-4 bg-surface-900/60">
